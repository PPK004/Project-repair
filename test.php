...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['title'])) {
        ...
        $sql = "INSERT INTO notifications (title, description, location, scheduled_datetime, status ,repair_id) 
                VALUES ('$title', '$description', '$location', '$scheduled_datetime', 'pending', '$rep_id')";
        if ($conn->query($sql) === TRUE) {
            echo "<script>alert('การแจ้งเตือนถูกบันทึกเรียบร้อยแล้ว!');</script>";
        } else {
            echo "<script>alert('เกิดข้อผิดพลาด: " . $conn->error . "');</script>";
...
$repair_sql = "SELECT id, repair_name, description, location, report_date FROM repair_request ORDER BY report_date DESC";
...
<div class="container mx-auto p-8">
    <div class="mb-4">
        ...
        <a href="dashboard.php" class="bg-green-500 text-white p-2 rounded mr-2">แดชบอร์ด</a>
